...
implementation Ravel = {
    ...
    predicate ravel_cond(input: Array, ix_space: IndexContainer, flat: Array, c: UInt32) {
        value uint_elem(c) < uint_elem(total(input));
    }

    procedure ravel_body(obs input: Array, obs ix_space: IndexContainer, upd flat: Array, upd c: UInt32) {

        var multi_ix = get_index_ixc(ix_space, c);
        var linear_ix = create_index1(c);

        call set(flat, linear_ix, unwrap_scalar(get(input, multi_ix)));
    ...
    function ravel(a: Array): Array = {
        ...
        call rav_repeat(a, ix_space, flat, c);
