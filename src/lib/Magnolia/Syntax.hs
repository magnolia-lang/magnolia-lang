...
type RequirementMap = M.Map TcDecl TcDecl
...
data ExternalDeclDetails =
  ExternalDeclDetails { -- | The backend associated with the declaration
                      ...
                      , _externalDeclElementName :: Name
                        ...
                        -- considered.
                      , _externalDeclRequirements :: RequirementMap
                      }
  deriving (Eq, Ord, Show)
...
data ConcreteDeclOrigin =
    ConcreteDeclOrigin DeclOrigin (Either RequirementMap ExternalDeclDetails)
  | GeneratedBuiltin
    deriving (Eq, Show)
...
mkConcreteLocalDecl :: Maybe (Backend, FullyQualifiedName)
                    -> [TcDecl] -- ^ requirements on the concrete declaration
                    -> SrcCtx
                    -> Name
                    -> MgMonad ConcreteDeclOrigin
mkConcreteLocalDecl mexternalInfo requirements src name =
  case mexternalInfo of
    Nothing -> pure $ ConcreteLocalMagnoliaDecl src
      (M.fromList (map (\d -> (d, d)) requirements))
    Just (backend, fqn) ->
      mkExternalDeclDetails (backend, fqn, requirements) src name
...
concreteDeclOriginRequirements conDeclO = case conDeclO of
  GeneratedBuiltin -> []
  ConcreteMagnoliaDecl _ requirements -> M.elems requirements
...
transformRequirements f conDeclO = case conDeclO of
  ...
  ConcreteMagnoliaDecl declO requirements ->
    let newRequirements = M.map f requirements
    in ConcreteMagnoliaDecl declO newRequirements
...
instance HasSrcCtx ConcreteDeclOrigin where
  srcCtx (ConcreteDeclOrigin declO _) = srcCtx declO
...
pattern ConcreteExternalDecl declO extDeclDetails =
  ConcreteDeclOrigin declO (Right extDeclDetails)

pattern ConcreteMagnoliaDecl :: DeclOrigin
                             -> RequirementMap
                             -> ConcreteDeclOrigin
pattern ConcreteMagnoliaDecl declO requirementMap =
  ConcreteDeclOrigin declO (Left requirementMap)

{-# COMPLETE ConcreteExternalDecl, ConcreteMagnoliaDecl, GeneratedBuiltin #-}

pattern ConcreteImportedMagnoliaDecl
  :: FullyQualifiedName -> SrcCtx -> RequirementMap -> ConcreteDeclOrigin
pattern ConcreteImportedMagnoliaDecl fqn src requirementMap =
  ConcreteDeclOrigin (ImportedDecl fqn src) (Left requirementMap)

pattern ConcreteLocalMagnoliaDecl :: SrcCtx
                                  -> RequirementMap
                                  -> ConcreteDeclOrigin
pattern ConcreteLocalMagnoliaDecl src requirementMap =
  ConcreteDeclOrigin (LocalDecl src) (Left requirementMap)

pattern ConcreteLocalExternalDecl :: ExternalDeclDetails
                                  -> SrcCtx
                                  -> ConcreteDeclOrigin
pattern ConcreteLocalExternalDecl extDeclDetails src =
  ConcreteDeclOrigin (LocalDecl src) (Right extDeclDetails)
